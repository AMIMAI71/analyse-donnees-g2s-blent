...
st.header(
    "Carte interactive de réparition des accidents corporels de la circulation routière"
)
...
with st.sidebar:
    ...
    if selectedDep != "Tous":
        ...
        filteredCaracteristiques = dataCaracteristiques[
            dataCaracteristiques["dep"] == selectedDep
        ]
    else:
        filteredCaracteristiques = dataCaracteristiques
    ...
    conditionAtmospheriqueSelected = st.sidebar.selectbox(
        label="Condition athmosphérique", options=conditionsAtomospheriques
    )
    if conditionAtmospheriqueSelected != "Tous":
        ...
        filteredCaracteristiques = filteredCaracteristiques[
            filteredCaracteristiques["atm"] == conditionAtmospheriqueSelected
        ]

    conditionsEclairages = sorted(LUM.values())
    conditionsEclairages = add_all_option(conditionsEclairages)
    conditionEclairageSelected = st.sidebar.selectbox(
        label="Luminosité", options=conditionsEclairages
    )
    ...
    localisationSelected = st.sidebar.selectbox(
        label="Localisation", options=localisation
    )
    if localisationSelected != "Tous":
        ...
        filteredCaracteristiques = filteredCaracteristiques[
            filteredCaracteristiques["agg"] == localisationSelected
        ]
    ...
    if selectedColision != "Tous":
        ...
        filteredCaracteristiques = filteredCaracteristiques[
            filteredCaracteristiques["col"] == selectedColision
        ]

    categories = sorted(ROUTE_TYPE.values())
    categories = add_all_option(categories)
    selectedCategorieRoute = st.selectbox(
        label="Catégorie de route", options=categories
    )
    ...
    dataJoinedAndFiltered = filteredCaracteristiques.join(
        filteredLieuxByCatr, how="inner"
    )
...
joined_df = filteredCaracteristiques.join(
    dataVehicules.set_index("Num_Acc"), on="Accident_Id"
)
nombrevehiculeimpliquedyna = joined_df["id_vehicule"].nunique()

# Le nombre d'usagers impliqués.
joined_df = filteredCaracteristiques.join(
    dataUsagers.set_index("Num_Acc"), on="Accident_Id"
)
...
df = pd.DataFrame({"col1": dataPosition["lat"], "col2": dataPosition["long"]})
